...
func TestPokerSet2Basic(t *testing.T) {
	...
	ps = 0x1111100
	if ps.SubtractBestEffort(0x21000) != 0x1100100 {
		t.Error("PokerSet2 SubtractBestEffort incorrect")
	}
}
...
func Test_parse(t *testing.T) {
	...
	ps = 0x100000000000000
	pkCnt, sCnt, _, _, _, sList, _, _, _ = ps.parse()
	fmt.Println(pkCnt, sCnt, sList)
	if pkCnt != 1 || sCnt != 1 || sList[0][0] != 14 || sList[0][1] != 1 {
		t.Error("PokerSet2 parse incorrect2")
	}
}
