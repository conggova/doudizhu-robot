...
func Test_getAffsSingle(t *testing.T) {
	affs := getAffsSingle(0x321, 0, 4)
	if len(affs) != 5 {
		t.Error("getAffsSingle incorrect")
	}
	affs = getAffsSingle(0x21200100010, 0, 3)
	if len(affs) != 18 {
		t.Error("getAffsSingle incorrect")
	}

	affs = getAffsSingle(0x21200100010, 0, 8)
	if len(affs) != 18 {
		t.Error("getAffsSingle incorrect")
	}
}

func Test_getAffsCouple(t *testing.T) {
	affs := getAffsCouple(0x321, 2)
	fmt.Println(affs)
	if len(affs) != 1 {
		t.Error("getAffsCouple incorrect")
	}
	affs = getAffsCouple(0x212234100010, 3)
	fmt.Println(affs)
	if len(affs) != 18 {
		t.Error("getAffsCouple incorrect")
	}
...
func Test_possibleActionsWithContext(t *testing.T) {
	...
	ps = 0x3000021
	a := NewActionWithoutAff(Triple, 1, 5).SetAff(0x2)
	fmt.Println(a, a.PokerSet2())
	actions = ps.possibleActionsWithContext(a)
	fmt.Println(actions)
	if len(actions) != 1 {
		t.Error("possibleActionsWithoutContext incorrect")
	}
